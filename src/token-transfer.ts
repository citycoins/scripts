...
import { cancelPrompt, debugLog, disclaimerIntro, exitError, exitSuccess, printDivider } from "../lib/utils"
import { getNonce, MICRO_UNITS, STACKS_NETWORK } from "../lib/stacks"
import { getCCBalance } from "../lib/citycoins"
import { bufferCVFromString, noneCV, someCV, standardPrincipalCV, uintCV } from "micro-stacks/clarity"
import { PostConditionMode, makeStandardFungiblePostCondition, FungibleConditionCode, createAssetInfo, makeContractCall, SignedContractCallOptions, AnchorMode, broadcastTransaction } from "micro-stacks/transactions"
...
export async function promptUser() {
  ...
  const userConfig = await prompts([
    ...
    },
    {
      ...
      validate: (value: string) => (value === "" ? "Stacks address is required" : true),
    },
    {
      type: "password",
      name: "stxPrivateKey",
...
export async function tokenTransfer(config: any) {
  ...
  console.log(`Amount: ${(config.transferAmount / MICRO_UNITS).toLocaleString(undefined, { minimumFractionDigits: 6, maximumFractionDigits: 6 })} ${config.citycoin}`)
  ...
  const memoCV = config.transferMemo.length > 0 ? someCV(bufferCVFromString(config.transferMemo)) : noneCV()
  // create tx options
...
disclaimerIntro("Token Transfer", "Builds and submits a CityCoin token transfer on Stacks", true)
